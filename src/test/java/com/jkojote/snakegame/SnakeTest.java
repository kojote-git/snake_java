...
public class SnakeTest {
    ...
    public void move_MoveSnakeAndCheckWhetherItsHeadHasExpectedPosition() {
        FieldCell prevPosition, expectedPosition, currentPosition;
        int speed;

        snake = new Snake(new FieldCell(10, 10), Direction.RIGHT, 4);
        speed = snake.getSpeed();
        prevPosition = snake.getHead().getPosition();
        snake.move();
        expectedPosition = new FieldCell(prevPosition.getX() + speed, prevPosition.getY());
        currentPosition = snake.getHead().getPosition();
        assertEquals(expectedPosition, currentPosition);

        snake.changeDirection(Direction.UP);
        prevPosition = currentPosition;
        expectedPosition = new FieldCell(prevPosition.getX(), prevPosition.getY() - speed);
        snake.move();
        currentPosition = snake.getHead().getPosition();
        assertEquals(expectedPosition, currentPosition);
    ...
    public void move_MoveSnakeUntilItEatsItself() {
        snake = new Snake(new FieldCell(10, 10), Direction.RIGHT, 8);
        snake.changeDirection(Direction.UP);
        snake.move();
        snake.move();
        snake.changeDirection(Direction.LEFT);
    ...
    @Test
    public void grow() {
        snake = new Snake(new FieldCell(10, 10), Direction.RIGHT, 3);
